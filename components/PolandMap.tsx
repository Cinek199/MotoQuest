...
export default function PolandMap() {
  ...
  useEffect(() => {
    const load = () => {
      try {
        ...
        setCounts(JSON.parse(localStorage.getItem("mq_voivodeship_tiles") || "{}"));
      } catch {
        setDiscovered([]);
        setCounts({});
      }
    ...
    load();
    window.addEventListener("mq-voivodeships-updated", load);
    window.addEventListener("storage", load);

    return () => {
      window.removeEventListener("mq-voivodeships-updated", load);
      window.removeEventListener("storage", load);
    };
  }, []);

  const discoveredKeys = useMemo(
    () => new Set(discovered.map(normalize)),
    [discovered]
  );

  const percentFor = (region: Region) =>
    Math.min(
      100,
      Math.max(discoveredKeys.has(normalize(region.name)) ? 3 : 0, Math.round((counts[region.slug] || 0) / 4))
    );

  const countryPercent = Math.round(
    REGIONS.reduce((sum, region) => sum + percentFor(region), 0) / 16
  );

  const discoveredCount = REGIONS.filter((region) => percentFor(region) > 0).length;

  return (
    <section className="mq-poland-map-panel">
      <div className="mq-poland-map-head">
        <div>
          <span className="mq-profile-eyebrow">Postep kraju</span>
          <h2>Odkryta Polska</h2>
          <p>
            Herby wojewodztw ujawniaja sie wraz z postepem, a mapa pokazuje
            aktualne tempo odkrywania.
          </p>
        </div>

        <div className="mq-poland-map-pill">
          <strong>{countryPercent}%</strong>
          <span>Polski</span>
        </div>
      </div>

      <div className="mq-poland-map-summary">
        <MapSummaryCard label="Wojewodztwa" value={`${discoveredCount}/16`} />
        <MapSummaryCard label="Najblizej 100%" value={bestRegionLabel()} />
      </div>

      <div className="mq-poland-map-stage">
        <svg viewBox="0 0 400 430" role="img" aria-label="Mapa wojewodztw Polski">
          <defs>
            {REGIONS.map((region) => (
              <clipPath id={`clip-${region.slug}`} key={region.slug}>
          ...
          {REGIONS.map((region) => {
            const percent = percentFor(region);
            const active = selected?.slug === region.slug;

            return (
              <g
                key={region.slug}
                onClick={() => setSelected(region)}
                className={["mq-polish-region", active ? "is-active" : ""].join(" ")}
              >
                <polygon
                  points={region.points}
                  fill={percent > 0 ? "rgba(24,24,27,0.88)" : "#121316"}
                  stroke={percent > 0 ? "#ff7900" : "#4a4d54"}
                  strokeWidth={active ? 4 : percent > 0 ? 3 : 2}
                />
                <image
                  href={`/voivodeships/${region.slug}.svg`}
                  x={region.x}
                  y={region.y}
                  width={region.w}
                  height={region.h}
                  preserveAspectRatio="xMidYMid meet"
                  clipPath={`url(#clip-${region.slug})`}
                  opacity={percent / 100}
                />
              </g>
            );
      ...
      {selected ? (
        <div className="mq-region-detail-card">
          <div className="mq-region-detail-copy">
            <img src={`/voivodeships/${selected.slug}.svg`} alt="" />
            <div>
              <strong>{selected.name}</strong>
              <small>Odkryto {percentFor(selected)}%</small>
            </div>
          </div>
  ...
  function bestRegionLabel() {
    const sorted = [...REGIONS].sort((left, right) => percentFor(right) - percentFor(left));
    const region = sorted[0];

    if (!region || percentFor(region) === 0) {
      return "Brak";
    }

    return `${region.name} ${percentFor(region)}%`;
  }
}

function MapSummaryCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="mq-poland-map-summary-card">
      <span>{label}</span>
      <strong>{value}</strong>
    </div>
  );
